...
function copy_path(src,dst,opt,fn){
  ...
  if (!opt.clobber) opt.clobber = true;  //if set to false, copy will not overwrite destination files that already exist.
  ...
  fs.exists(src,function(exists){
	  ...
	  fs.readdir(src, function(err,files){
	      ...
	      console.log(c_files);
	      work_arr(src,dst,opt,c_files,0,fn);
	  });
      });
  });
  ...
  function next(src_file,dst_file,opt,cb) {
	  ...
	  if (stats.isFile()) {
	    copy_file(src_file,dst_file,opt.clobber,cb);
	  }else{
	    copy_path(src_file,dst_file,opt,cb);
	  }
      });
  }
}
