...
function var_dump_node(obj_name,obj,options,level){
    ...
    if(options.exclude instanceof Array){
        for(var i in options.exclude){
            ...
            if(exclude_opt instanceof RegExp){
                if(exclude_opt.test(obj_name)) return obj_name + " = {/*exclude: " +exclude_opt+ "*/};"+options.separator;
            }else
            if(typeof(exclude_opt) === 'string' || exclude_opt instanceof String){
                if(exclude_opt == obj_name) return obj_name + " = {/*exclude: " +exclude_opt+ "*/};"+options.separator;
            }else{
                console.log('var_dump_node: undefined exclude type ' + typeof exclude_opt+ '  == '+ exclude_opt);
            }
        }
    }else{
        var exclude_opt = options.exclude[i];
  ...
  if(typeof obj == 'function'){
    ...
    ret += obj_name+" = \"" +s+"\"; //length:"+len+options.separator;
  }else if(typeof obj == 'number' ){
    var n = Number(obj);
    if(typeof options.prepare_num_function == 'function'){
        n = options.prepare_num_function(n);
    }
    ret += obj_name+" = " +n+";"+options.separator;
  }else if(typeof obj == 'boolean') ret += obj_name+" = "+obj+";"+options.separator;
  ...
  else if(obj instanceof Object && typeof obj == 'function' /*&& obj.toString().match(/^[\s\n]*function/)*/){
  ...
  if(ret==""){
    if(typeof obj == 'string' && obj.charAt(0)=="/") ret += obj_name+" = "+obj+";"+options.separator;
    else{
...
function get_object_size(obj){
  ...
  for (key in obj) { // check the okeys in the object
    if (obj.hasOwnProperty && obj.hasOwnProperty(key)) size++; // increase the size
  }
